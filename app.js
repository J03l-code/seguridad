      ...
      try {
        ...
        showModal(`
          ...
          <div class="modal-body">
            ...
            ${act.length > 0 ? `<div><label class="form-label">Historial y Comentarios</label><div class="activity-list">${act.map(a => `<div class="activity-item"><div class="activity-avatar"style="width:28px;height:28px;font-size:10px">${initials(a.user_name)}</div><div style="flex:1"><div class="activity-text"style="font-size:13px"><strong>${a.user_name}</strong> ${a.action === 'commented' ? `<div style="margin-top:4px;padding:8px;background:var(--gray-50);border-radius:6px;border:1px solid var(--gray-200);color:var(--gray-800);white-space:pre-wrap;word-break:break-all">${a.details.replace(new RegExp('(https?://[^\\\\s]+)', 'g'), '<a href="$1" target="_blank" style="color:var(--primary-600);text-decoration:underline">$1</a>')}</div>` : a.details}</div><div class="activity-time">${formatDate(a.created_at)}</div></div></div>`).join('')}</div></div>` : ''}
